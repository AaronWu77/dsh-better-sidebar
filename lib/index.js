...
function readPrefs(config) {
	...
	for (const key of Object.keys(prefs)) {
		...
		if (value !== void 0) prefs[key] = value;
	}
	return prefs;
}
z.object({
...
function apply(ctx, config) {
	...
	ctx.inject(["settings"], (injected) => {
		const sctx = injected;
		ctx.effect(() => sctx.settings.configure({ auto: false }, ctx.fiber), "dsh-better-sidebar: custom settings page policy");
		const ownNs = () => {
			...
			if (declared !== void 0) return declared.ns;
		...
		const readCurrent = () => readPrefs(config);
		const scope = {
			get: readCurrent,
			watch: (callback) => {
				let previous = readCurrent();
				return ctx.on("settings/document-updated", (updatedNs) => {
